import re, os, glob
root = os.getcwd()
matches = glob.glob(os.path.join(root, 'articles v2/mds/*TS-RAG*'))
fpath = matches[0]
with open(fpath, 'r', encoding='utf-8') as fh: lines = fh.readlines()
...
for s in secs:
    ...
    print(f'{s["start"]:4d}-{e:<4d} | {s["level"]:3d} | {s["title"][:60]:60s} | {tok("".join(s["lines"])):5d}')
print(f'Total: {len(secs)} blocks, ~{sum(tok("".join(s["lines"])) for s in secs)} tok')
