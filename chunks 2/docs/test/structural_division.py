...
root = os.getcwd()
matches = glob.glob(os.path.join(root, 'articles v2/mds/*TEST*TEXT*'))
fpath = matches[0]
...
print(f'Lines | Lvl | Section                                     | Tok')
for s in secs:
    e = s['start']+len(s['lines'])-1
    print(f'{s["start"]:4d}-{e:<4d} | {s["level"]:3d} | {s["title"][:45]:45s} | {tok("".join(s["lines"])):5d}')
